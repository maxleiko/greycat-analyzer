...
//! capability handlers + ide services. LSP positions are 0-indexed
//! `(line, character)`; the rest of the codebase treats `character` as
//! a byte column (matching tree-sitter's `Point.column`). All
//! conversions go through this module so the convention stays
//! consistent.

use std::ops::Range;

use greycat_analyzer_core::lsp_types::{self, Position};
...
pub fn byte_range_to_lsp(text: &str, range: &Range<usize>) -> lsp_types::Range {
    lsp_types::Range {
