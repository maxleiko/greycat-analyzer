...
fn check_fn_inferred_return(
    ...
    let Some(ret_ty) =
        crate::return_inference::inferred_return_from_body(hir, analysis, arena, body)
    else {
        return;
    };
...
// `inferred_return_from_body`, `join_return_types`,
// `collect_return_types`, `collect_returns_in_block`, and
// `is_expressible_type_ident` moved to `crate::return_inference` so the
// `Expr::Lambda` typing arm can consume the exact same logic. A lambda
// is a top-level fn in a scope — one implementation, no second copy.
...
"#,
    ...
    fn unused_generics(src: &str) -> Vec<String> {
        lint(src)
            .into_iter()
            .filter(|d| d.rule == "unused-generic-param")
            .map(|d| d.message)
            .collect()
    }

    #[test]
    fn unused_generic_param_on_type_with_unused_t() {
        let diags = unused_generics("type Foo<T> { a: int; }\n");
        assert_eq!(diags.len(), 1, "expected one diag, got: {diags:?}");
        assert!(diags[0].contains("`T`"));
    }

    #[test]
    fn unused_generic_param_silent_when_type_uses_t_in_attr() {
        let diags = unused_generics("type Foo<T> { a: T; }\n");
        assert!(diags.is_empty(), "expected none, got: {diags:?}");
    }

    #[test]
    fn unused_generic_param_silent_when_supertype_uses_t() {
        let diags = unused_generics("type Bar<T> { x: T; }\ntype Foo<T> extends Bar<T> {}\n");
        assert!(diags.is_empty(), "expected none, got: {diags:?}");
    }

    #[test]
    fn unused_generic_param_silent_when_method_uses_t() {
        let diags = unused_generics("type Foo<T> { fn get(x: T): T { return x; } }\n");
        assert!(diags.is_empty(), "expected none, got: {diags:?}");
    }

    #[test]
    fn unused_generic_param_on_fn_with_unused_t() {
        let diags = unused_generics("fn foo<T>(x: int): int { return x; }\n");
        assert_eq!(diags.len(), 1, "expected one diag, got: {diags:?}");
        assert!(diags[0].contains("`T`"));
    }

    #[test]
    fn unused_generic_param_silent_when_fn_uses_t_in_param() {
        let diags = unused_generics("fn foo<T>(x: T): T { return x; }\n");
        assert!(diags.is_empty(), "expected none, got: {diags:?}");
    }

    #[test]
    fn unused_generic_param_skips_underscore_prefix() {
        let diags = unused_generics("type Foo<_T> { a: int; }\n");
        assert!(
            diags.is_empty(),
            "expected `_T` to be silently skipped, got: {diags:?}"
        );
    }

    #[test]
    fn unused_generic_param_skips_native_type() {
        let diags = unused_generics("native type Array<T> {}\n");
        assert!(
            diags.is_empty(),
            "native types are exempt — runtime backs the generic"
        );
    }

    #[test]
    fn unused_generic_param_flags_first_of_two_when_only_second_used() {
        let diags = unused_generics("type Map<K, V> { a: V; }\n");
        assert_eq!(diags.len(), 1, "expected only `K` flagged: {diags:?}");
        assert!(diags[0].contains("`K`"));
    }
