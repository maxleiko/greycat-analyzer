...
fn lambda_call_arg_diags(
    ...
    for i in 0..expected {
        ...
        if !index.is_assignable_to(arena, arg_ty, declared_ty) {
...
fn is_slot_assignable(
    ...
    if index.is_assignable_to(arena, from, to) {
        return true;
    }
...
fn check_construction_value_against_slot(
    ...
    if !is_slot_assignable(index, arena, value_ty, expected_ty) {
        ...
        .get(&value_expr)
        .copied()
        && !is_slot_assignable(index, arena, runtime_ty, expected_ty)
    {
        let r = cur_module.hir.exprs[value_expr].byte_range();
    ...
    for (_call_id, call_expr) in cur_module.hir.exprs.iter() {
        ...
        for i in 0..pair_count {
            ...
            if !index.is_assignable_to(arena, arg_ty, declared_ty) {
                ...
                .get(&call.args[i])
                .copied()
                && !index.is_assignable_to(arena, runtime_ty, declared_ty)
            {
...
fn collect_object_field_diags_split(
    ...
    for (obj_expr_id, expr) in cur_module.hir.exprs.iter() {
        ...
        for f in obj_expr.fields.iter() {
            ...
            if !index.is_assignable_to(arena, value_ty, declared_ty) {
    ...
    fn validate_stmt(
        ...
        match &hir.stmts[stmt_id] {
            ...
            Stmt::Return(r) if r.value.is_some() => {
                ...
                if let Some(rt) = return_ty {
                    ...
                    if let Some(runtime_ty) = analysis.expr_runtime_types.get(&v).copied()
                        ...
                        && index.is_assignable_to(arena, value_ty, rt)
                        && !index.is_assignable_to(arena, runtime_ty, rt)
                    {
                        let want =
            ...
            Stmt::Return(r) => {
                ...
                if let Some(rt) = return_ty {
                    let null_ty = arena.null();
                    if !index.is_assignable_to(arena, null_ty, rt) {
                        diags.push(SemanticDiagnostic {
                            severity: Severity::Error,
                            code: "type-mismatch",
                            message: format!(
