//! Debug dumpers exposed only to the playground UI.
//!
//! Each function renders one analyzer stage to JSON so the playground's
//! per-panel viewer can walk it as a normal object: tree-sitter CST,
//! HIR, tokens, expression types, merged diagnostics, formatter output.
//!
//! These are intentionally gated behind the `playground` cargo feature
//! so the published `@greycat/analyzer` wasm bundle (built with the
//! default feature set) stays small — the dumpers add ~50 KB of JSON-
//! serialization code that no Monaco / npm consumer needs.
