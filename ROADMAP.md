...
- [ ] **19.9 Project-wide identifier interning** (L) — current project-wide indexes (`ProjectIndex.{type_members, fn_signatures, enum_types, decl_locations, values, exposed, ...}`, `TypeMembers.{attrs, methods, attr_types, method_returns, generics}`, `FnSignature.generics`) all key on `String`. The original ident text lives behind `Idx<Ident>` per HIR — an `Idx` is `Copy` and 32-bit, but it's *not* comparable across modules (each HIR has its own ident arena). To unify, introduce a project-wide string interner (e.g. `lasso::Spur` or roll our own `Symbol(u32)` keyed off `ProjectIndex.symbols: HashMap<String, Symbol>`). Replace every `String` map key in project-wide indexes with `Symbol`. Each `Hir` keeps `Idx<Ident>`; the HIR lowering registers an `Symbol` per ident text. Drops a *lot* of `.clone()` / `.to_string()` in hot lookup paths. Cuts `lower_signatures_into` allocation overhead (the `generics_in_scope: HashMap<String, GenericOwner>` pattern + repeated `hir.idents[*g].text.clone()` loops become cheap `Symbol → GenericOwner` lookups). Out of scope for the staged-pipeline rewrite itself — the indexes need to land first so the interner can replace them surgically.
