...
- **15.2 Completion capability — base implementation** (L) — LSP `textDocument/completion`. The TS reference at [packages/server/src/index.ts:401](file:///tmp/lang/lang/packages/server/src/index.ts) (server hookup) and [packages/lang/src/project/analysis_result.ts:738](file:///tmp/lang/lang/packages/lang/src/project/analysis_result.ts) (`AnalysisResult.completion`) shows the minimal cases. Split into sub-chunks so each one is bisectable:
  ...
  - [ ] **15.2.2 Keyword completion at statement / expression positions** (S) — empty-prefix or alpha-prefix at statement/expression positions emits `fn`, `if`, `else`, `for`, `while`, `do`, `var`, `return`, `throw`, `try`, `catch`, `is`, `as`, `null`, `true`, `false`, `this`, `new`. Position detection via the cursor's containing CST kind (block / module / expr position). Filter by typed prefix.
  - [ ] **15.2.3 Scope-aware ident completion** (M) — uses cached `ProjectAnalysis::Resolutions` + walking each `Scope` ancestor up to module scope. Emits locals / params / generic params / in-module decls, plus `ProjectIndex::values` / `decl_locations` keys / `BUILTIN_RUNTIME_TYPES` / primitives at module-fallback. `CompletionItemKind` per `Definition` shape (Local → VARIABLE, Param → VARIABLE, Decl::Fn → FUNCTION, Decl::Type → CLASS, Decl::Enum → ENUM, Decl::Var → VARIABLE, Generic → TYPE_PARAMETER, ProjectDecl → resolve through foreign module, Project → MODULE/CLASS/FUNCTION as known). Filter by typed prefix.
  - [ ] **15.2.4 Member completion after `.` / `->`** (M) — trigger chars `.` and `>` (after `-`). Resolves the receiver's type via `AnalysisResult::expr_types`, walks `type_decls[name]` for attrs + methods. Cross-module receivers consult `foreign_member_lookup` / `decl_locations`. Auto-deref via `@deref`'s named method (`type_flags`).
  - [ ] **15.2.5 Static completion after `::`** (M) — trigger char `:` (require previous char to also be `:`). Two cases: (a) `Type::|` enumerates the type's static methods; (b) `module::|` enumerates the foreign module's top-level decls. Routes through `ProjectIndex::module_uri` for the module case and `decl_locations` + cached `ModuleAnalysis::hir` for the type case.
  - [ ] **15.2.6 Type-position completion** (S) — cursor inside a `type_ident` slot (e.g. `var x: |`, `<|`, `extends |`, `(p: |`, `): |`). Emits only type-shaped names: in-module type / enum decls + `ProjectIndex::registry` keys + `BUILTIN_RUNTIME_TYPES`. Filter by typed prefix.
  - [ ] **15.2.7 Object literal field completion** (S) — cursor inside an `object_initializers` block (`Type { | }` or `Type { x: 1, | }`). Resolves the object's type (the `type_ident` head of `object_expr`) and emits its `attrs` as `CompletionItemKind::FIELD`, skipping ones already present in the literal.
