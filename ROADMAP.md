...
- [ ] **34.1 Add `notify` dep + per-Backend watcher thread** (S) — spawn in `Backend::initialized` (or lazily on first project load). Channel into the main loop. Plumb `WatcherEvent { uri, kind: Create | Modify | Remove }`.
...
- [ ] **41.5b `CompletionItem` IDE ADT migration** (M) — move the rich completion ADT shape (`CompletionList`, `CompletionItem`, `CompletionItemKind` enum, `InsertTextFormat`, `Documentation` / `MarkupContent` flattening, `CompletionTextEdit::{Insert,Replace}` flattening, optional `additionalTextEdits` / `command`) out of `lsp_types` into `analysis::ide::completion`. The production logic already lives in [`analysis::ide::completion::completion_with_project`](greycat-analyzer-analysis/src/ide/completion.rs) — what changes is the return shape and ~190 internal construction sites. Server `capabilities/mod.rs` keeps the `completion_with_project` re-export with a thin `lsp_types::CompletionList` converter on top. Wasm gets a new `Project::completion(uri, line, character)` method returning the IDE-shape `CompletionList`. Also covers the `LibVersionPayload` registry-resolved completion path (already exposed from the analysis side; just swap the embedded `lsp_types::Range` for the IDE `Range`).
