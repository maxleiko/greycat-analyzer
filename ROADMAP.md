...
- [ ] **19.8 Workspace `.clone()` / `.to_string()` audit** (M) — sweep through every crate to remove unnecessary owned-allocation patterns. The borrow-checker bumps from P19-P23 added several reflexive `.clone()` / `.to_string()` calls in places where a borrow would have worked (split borrows, pre-binding fields, restructuring loops). Per the global rule in [`~/.claude/CLAUDE.md`](https://github.com/anthropics/claude-code/issues): default to borrowing, allocate only when ownership is genuinely needed. Target: zero `String::clone` / `String::to_string` / `Vec::clone` calls in hot paths (analyzer body walker, project orchestrator, capability handlers) without an inline comment explaining the constraint.
