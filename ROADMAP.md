...
- [ ] **5.2 Playground as analyzer testbed** (M, **backend ready**) — wasm exports for every analyzer stage are in place (P5.1). The playground UI itself (`greycat-analyzer-playground/`) is a Vite/TS app and is gitignored — its `parse_sexp`, `parse_tree`, `tokens`, `lower_hir`, `infer_types`, `diagnostics`, `format` panels are a separate frontend deliverable. Backend half done; promotion of the UI is the remaining work.
