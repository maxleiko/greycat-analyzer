...
- [ ] **14.9 `private` semantics correction** (M) — GreyCat's `private` means "must be accessed via the fully-qualified name", **not** "module-only" as in most other languages. The supported qualified-access shapes are `<module>(::<type>)?::<name>(::<field>)?` — module-prefixed references against the bare module name, optionally chaining through a type and a field; bare filenames (`runtime.gcl::…`) are **not** valid. Today's `UnusedDecl` lint (P6.7) treats `private` as module-private and flags every `private` decl whose intra-module ref count is zero — false-positives for any decl that's only used through its qualified name (the 7 residual `unused private … runtime.gcl` warnings on stdlib lint are exactly this). Fix needs two pieces: (a) the resolver / cross-module reference walker recognize the `<module>::<name>` (and `<module>::<type>::<name>`) shapes that the grammar produces (`Expr::Static` heads → `module_name`, then `member_name`) and bump `references_to` for the matching foreign decl; (b) the `UnusedDecl` rule scopes "ref count zero" to "no qualified or unqualified reference *anywhere in the project*" rather than just the declaring module. After this lands, the stdlib lint baseline should drop from 7 to 0.
