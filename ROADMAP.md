...
- [ ] **15.2 Completion capability — base implementation** (L) — LSP `textDocument/completion` is currently unimplemented and `completion_provider` is not advertised in `initialize`. Foundational `capabilities::completion` handler:
  ...
  - **Static / FQN completion** after `Type::` — consult the foreign type's `methods` (gates on **P15.6**, which lands the static_expr cross-module resolver/binding plumbing).
  ...
  - **Gates on P15.6 + P15.7.** Completion is the consumer of resolution / inference data — until `Identity::create` resolves to a foreign method (P15.6) and `var x = Identity::create(...)` infers `x: Identity` instead of `any` (P15.7), the completion list will be wrong on real GreyCat code (the entire stdlib uses `Type::method` static-call shape pervasively). Land 15.6 + 15.7 first, then 15.2 ratchets up cleanly.
...
- [ ] **15.6 Cross-module `static_expr` resolution (FQN goto-def + binding)** (M) — `Identity::create("root", "root")` (a `static_expr` whose `Type` lives in another module — runtime.gcl in stdlib) currently produces no goto-def hit and no member binding. Two pieces:
  - **Resolver / analyzer.** Extend `analysis::analyzer::resolve_member` (or its sibling `resolve_static_member`) so the `property` ident of a `static_expr` whose head resolves to `Definition::ProjectDecl { uri, decl: type_decl }` walks the foreign module's cached HIR `TypeDecl::methods`, finds the matching `name`, and records a `ForeignMember { uri, member: MemberDef::Method(decl) }` into `AnalysisResult::foreign_member_uses`. Today only `member_expr` (the `.` shape, P11.5) populates that map; `static_expr` (`::` shape) is silently skipped, which is why `Identity::create` doesn't bind.
  - **goto-def integration.** `goto_definition_handler` already consults `foreign_member_lookup` after the `ProjectDecl` branch (P11.5) and uses `cross_module_member_location` to compute the foreign attr/method name range. Once 15.6's resolver pass populates `foreign_member_uses` for static expressions, the existing handler picks them up — no LSP-side change needed beyond confirming the wiring.
  - **Tests.** New unit test in `lsp_capabilities.rs`: `Identity::create` in `main.gcl` against a `runtime.gcl` declaring `type Identity { static native fn create(...): Identity; }` should resolve to the foreign method's name range. Mirrors `cross_module_member_resolution_binds_foreign_attr` but for the `::` shape.

- [ ] **15.7 Static-call return-type inference** (S) — separate from 15.6: even once `Identity::create` *binds*, the analyzer's `Expr::Call` arm only consults `Decl::Fn::return_type` for in-module callees (and the P12.1 generic-inference path for those). Cross-module method calls fall through to `any?` because the analyzer doesn't reach into the foreign HIR's `Decl::Fn::return_type`. Wire the same lookup the analyzer does for in-module calls: when `Expr::Call` resolves to a `ForeignMember::Method(decl_id)` (the binding from P15.6), pull the foreign module's HIR via `ProjectIndex::locate_decl` (or the cached resolutions), read `decl.return_type`, lower it under the foreign module's generic scope (`generics_in_scope` if needed), and intern the result into `expr_types`. **Acceptance:** the inlay hint for `var x = Identity::create("root", "root");` should render `x: Identity`, not `x: any`. New unit test in `analyzer/tests/` (or `lsp_capabilities.rs`) — set up a 2-doc workspace with the foreign Identity type and assert the call-site's `expr_types` entry equals the foreign type's `Named("Identity")` interning. Generic substitution across modules (e.g. `Array<T>::create()` returning `Array<T>` where `T` is bound at the call site) is **deferred** to a follow-up — this chunk only covers concrete return types.

**M15: hover restores doc / signature / provenance; cross-module `Type::method` resolves and infers correctly; basic completion lights up in the LSP and the WASM/Monaco playground (sharing the same algorithm via a `RegistryFetcher` trait — native HTTP for the LSP, JS-callback fetch for WASM); `@library` version completion hits `get.greycat.io` in both targets and matches TS-reference behavior; `@include` directory completion is wired where the project root is known; `@include` / `@library` typos and duplicates are surfaced as diagnostics in both `cli lint` and the LSP.**
