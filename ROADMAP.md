...
- [x] **19.6 Query-DAG invalidation (P24)** (L, partial) — `invalidate(uri)` now correctly re-runs `lower_signatures_into` (S7-S11) project-wide on every edit, so the post-P22 signature index doesn't go stale when an attr / method / fn return type changes. The free-function `lower_signatures_into` takes split `&mut TypeArena` + `&mut ProjectIndex` borrows so the invalidate path can build the `(Uri, &Hir)` slice from references into `self.modules` — no `Hir` clone. Dead pass 3.5 / pass 3.52 calls removed from invalidate. Full Q1-Q5 query DAG (separate hashes per stage, per-Uri Q5 cascade filtering, signature-hash → skip S7-S11 short-circuit) deferred to a follow-up — today's invalidation is *correct* under P19-P23 but doesn't yet hit the < 50ms p99 target on a 50-file synthetic project. The infrastructure shape is in place; the per-stage hashing is the remaining work.
