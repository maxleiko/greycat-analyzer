...
- [x] **15.7 Static-expr / static-call type inference** (S) — new pass 3.5 in `ProjectAnalysis::rebuild` (between cross-module member resolution and the qualified-refs pass) infers types for three previously-`any` shapes:
  ...
  Two-phase: phase 1 walks every module's HIR read-only and snapshots the foreign return type into an arena-free `TypeShape` enum; phase 2 mints those shapes into each affected module's `TypeArena` and updates `expr_types[call_id]` and (for `var x = ...` initializers without a declared type) re-links `def_types[name]` so inlay hints / hover / downstream inference see the right type. Generic substitution across modules (e.g. `Array<T>::create()` returning `Array<T>` where `T` is bound at the call site) keeps the generic head as-is without binding `T` — that's a follow-up. 3 new tests in `lsp_capabilities.rs` cover the three shapes.

- [ ] **15.8 FQN `module::Type::member` lowering + inference** (M) — `runtime::Identity::create("root", "root")` (the FQN form prefixed with the module name) currently lowers to `Expr::Unsupported` because HIR's `StaticExpr { ty: TypeRef, property: Ident }` shape can't represent a static-expr nested in the `ty` slot — the grammar allows `static_expr :: property` (chained) but the HIR doesn't. Two pieces:
  - **HIR change.** Extend `StaticExpr` (or add a new `Expr::QualifiedStatic`) so the left side can be either a `TypeRef` *or* another `StaticExpr`. Update `lower::lower_expr`'s `static_expr` arm to walk chains. Capabilities that consume `Expr::Static` (analyzer member resolution, hover, goto-def, `compute_qualified_refs`) need adapter shims.
  - **Pass 3.5 extension.** Once `runtime::Identity::create` lowers to a chain, P15.7's `infer_cross_module_call_types` needs to follow the `module::Type` prefix to the foreign module by name (instead of just looking up `Type` in `ProjectIndex.decl_locations`). The `compute_qualified_refs` pass already walks chain shapes via the CST; reuse that resolution path.
  - **Acceptance:** `var x = runtime::Identity::create("a", "b");` infers `x: Identity` (matching the unprefixed form's behavior from P15.7). Goto-def on each segment of the chain hits the right target (`runtime` → P15.9, `Identity` → its `type Identity` decl, `create` → its method decl). HIR `Expr::Unsupported` regression test stays clean.
