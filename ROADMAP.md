...
- [x] **15.8 FQN `module::Type::member` lowering + inference** (M) — chained `static_expr` (`runtime::Identity::create`, `runtime::Identity::id`, etc.) used to lower to `Expr::Unsupported` because HIR's `StaticExpr` had `ty: Idx<TypeRef>` and the grammar's chained head is itself a `static_expr` rather than a `type_ident`. Two pieces:
  ...
  **Deferred:** goto-def / hover on chain segments 1 + 2 (the type and member parts — chain[0] already works via P15.9). Both need a new `Resolutions` / `AnalysisResult` field that binds chain segment idents to foreign decls; today they fall through to in-module HIR which doesn't see the foreign decl. Tracked under a follow-up.

- [ ] **15.10 Call-site argument-type validation** (S) — the analyzer's `Expr::Call` arm only enforces declared param types via the **generic-inference** path (`try_generic_call_inference`, P12.1). For non-generic callees — including the most common case, a plain `fn f(x: int)` called as `f(42)` — there's no `is_assignable_to(arg_ty, param_ty)` check, so `expect_Identity(42)` produces zero diagnostics today even though the call is obviously wrong. Add the pairwise check after generic inference returns `None` (or in addition, when generic inference fully substitutes): for each `(declared_param_ty, arg_ty)` pair, emit a `value of type X is not assignable to parameter Y` diagnostic on assignability mismatch. Apply to in-module fn callees, in-module method calls, cross-module method calls (foreign params translated via `read_type_shape` + the destination arena), and call-of-static. **Acceptance:** the `expect_Identity(42)` / `expect_fn(42)` / `expect_field(42)` / `expect_ty(42)` lines in the user's baseline produce a typed diagnostic at the offending arg's range.
