...
| File | Owns |
|---|---|
| [src/conv.rs](../greycat-analyzer-server/src/conv.rs) | Re-export facade. The implementation (`position_to_byte`, `byte_to_position`, `byte_range_to_lsp`, `ranges_overlap`, `stmt_byte_range`) lives in [`greycat_analyzer_analysis::conv`](../greycat-analyzer-analysis/src/conv.rs) so analysis-side ide services share it. Every Position ↔ byte-offset conversion goes through there so the `character == byte column` convention stays consistent. |
| [src/capabilities/mod.rs](../greycat-analyzer-server/src/capabilities/mod.rs) | Re-export hub only. No logic. ~50 lines. |
| [src/capabilities/hover.rs](../greycat-analyzer-server/src/capabilities/hover.rs) | `hover_with_project` + `hover_inner` fallback + the markdown builders that wrap rendered signatures with doc / member / provenance prose (`ident_hover_markdown`, `member_hover_markdown`, `foreign_member_hover_markdown`, `render_decl_hover_markdown`). The signature / type-ref renderers themselves (`render_decl_signature`, `render_fn_signature`, `render_type_ref(_with_subst)`, `RenderCtx`, `decl_doc`, `module_label_for_uri`) live in [`greycat_analyzer_analysis::ide::render`](../greycat-analyzer-analysis/src/ide/render.rs) so completion and signature_help consume them without crossing capability modules. |
...
**Capability services in the analysis crate live under [`src/ide/`](../greycat-analyzer-analysis/src/ide/).** When a capability's heavy lifting is *analysis* (scope walking, type member discovery, name binding queries, edit synthesis), the work lives in `greycat-analyzer-analysis/src/ide/<capability>.rs` and the LSP layer is shape-conversion only. The reference pattern is [`greycat_analyzer_analysis::ide::rename`](../greycat-analyzer-analysis/src/ide/rename.rs):
...
Currently in `ide/`: [`actions`](../greycat-analyzer-analysis/src/ide/actions.rs) (`CodeActionCategory` + `TextEdit` vocabulary), [`quickfix`](../greycat-analyzer-analysis/src/ide/quickfix.rs) (lint-fix / code-action edit synthesis — consumed by *both* the LSP server and the CLI's `lint --fix`), [`rename`](../greycat-analyzer-analysis/src/ide/rename.rs), [`render`](../greycat-analyzer-analysis/src/ide/render.rs) (signature / type-ref rendering shared by hover, completion, signature_help), [`completion`](../greycat-analyzer-analysis/src/ide/completion.rs) (scope / member / static / type-position / directive / pragma / library-version completion — still emits `lsp_types::CompletionItem` pending a `CompletionCandidate` ADT). **Hard rule: when adding a new LSP capability, ask "is this LSP shape work, or is it analysis?" If analysis, the implementation goes under `analysis::ide::` even on the first commit.**
...
| Auto-fix dispatch | [greycat-analyzer-analysis/src/ide/quickfix.rs](../greycat-analyzer-analysis/src/ide/quickfix.rs) (`edit_for_diagnostic` match + per-rule fn) | When the rule has an auto-fix. Both `lint --fix` and the LSP's `textDocument/codeAction` go through this single dispatch — there is no second one. |
...
| Touchpoint | File | When |
|---|---|---|
| **Decide where the logic lives.** Pure LSP shape work (e.g. folding ranges) → file in `capabilities/`. Analysis work (scope walking, name binding queries, member discovery, edit synthesis) → service in [`greycat-analyzer-analysis::ide::<capability>`](../greycat-analyzer-analysis/src/ide/), with a thin `capabilities/` wrapper. The [`analysis::ide::rename`](../greycat-analyzer-analysis/src/ide/rename.rs) ↔ [`capabilities::references_rename`](../greycat-analyzer-server/src/capabilities/references_rename.rs) split is the reference shape. | — | **Always.** If you can't decide in 30 seconds, default to analysis-side — it's easier to demote from analysis to LSP than to migrate a capability that already inlined LSP types into the analysis crate. |
...
If you find code excluding something on `is_private` that isn't one of those two specific cases, it's wrong. The defect that surfaced this rule: [index.rs:804](../greycat-analyzer-analysis/src/index.rs#L804) gated `type_members` insertion on `!is_private`, which broke inherited-member resolution for private types in their own module — the supertype walk in `type_attr_id_chain` / `walk_member_chain` couldn't even start, so `LabelDrawing.height` (with `private LabelDrawing extends ElementDrawing` declaring `height`) reported a spurious `unknown-member`, and `LabelDrawing { does_not_exists: "rekt" }` skipped the unknown-field check entirely.
